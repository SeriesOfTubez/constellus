...
def upgrade() -> None:
    ...
    op.create_index("ix_connector_configs_connector_id", "connector_configs", ["connector_id"])
    ...
    # TimescaleDB hypertables — partitioned on time for efficient time-series queries
    op.create_table(
        "assets",
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
