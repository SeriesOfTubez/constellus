...
async def saml_acs(request: Request, db: Session = Depends(get_db)):
    ...
    from app.services.saml import LocalAccountLinkBlocked
    ...
    except LocalAccountLinkBlocked:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="An account with this email already has local credentials. Contact an administrator to link SSO.",
        )
