...
`_probe_worker`'s real signature takes `db` and a required `asset_canonical_ids`
keyword (planning#205, the ownership-probe gate) — these fakes accept `db` and
ignore the gate-only kwargs via `**kw`, and `check_affinity` is called with
`db=None, asset_canonical_ids=[]` since the fake bypasses the gate entirely
(these tests are about `_score_port`'s scoring logic, not the gate).
...
def test_fully_dead_origin_sets_unreachable_votes():
    ...
    def fake_probe(db, hostname, origin_ip, ports=None, **kw):
        return {"ports": {
            "443": {"owned": {}, "default": {}},
            "80": {"owned": {}, "default": {}},
        }}
    da._probe_worker = fake_probe
    r = da.check_affinity(None, "test.example.com", "203.0.113.5", {"example.com"}, asset_canonical_ids=[])
