...
def _build_saml_settings(config) -> dict[str, Any]:
    ...
    return {
        "strict": True,
        "debug": False,
        # wantAssertionsSigned is the actual control that prevents a forged,
        # unsigned SAMLResponse from being accepted as authenticated — without
        # it, python3-saml validates a signature *if present* but doesn't
        # require one. Not requiring wantMessagesSigned too: Okta/Azure
        # AD/OneLogin sign the assertion by default but not always the outer
        # envelope, so forcing that would break common real IdP configs
