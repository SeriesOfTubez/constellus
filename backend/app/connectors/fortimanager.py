...
class FortiManagerConnector(EnrichmentConnector):
    ...
    def enrich(self, assets: list[DiscoveredAsset], config: dict[str, Any]) -> PhaseResult:
        ...
        Use connector_get() / connector_post() from app.connectors.http for all calls.
        """
        from app.connectors.http import connector_get, connector_post  # noqa: F401
        ...
        # Pattern:
        #   1. connector_post(f"{url}/vips", json={"ips": [...]}) → internal host mappings
        #   2. For each mapping, emit INTERNAL_HOST asset with parent = public IP
        #   3. connector_get(f"{url}/ips/findings") → emit findings for active IPS alerts
