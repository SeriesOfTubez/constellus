...
export function useFlyout<T extends { id: string }>(
  ...
  // J/K navigation steps through `items` (the visible list), but the selected
  // item is resolved against `lookupItems` when given — so a flyout can open for
  // something not in the visible list (e.g. an IP shown only in the Ports pivot,
  // which is a hidden child of its hostname row).
  const pool = lookupItems ?? items
